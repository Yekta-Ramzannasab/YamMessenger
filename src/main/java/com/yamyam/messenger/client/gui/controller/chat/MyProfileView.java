...
public final class MyProfileView {
    ...
    // Preferences node path (must be a String, not a Preferences instance)
    private static final String PREF_NODE_PATH = "com.yamyam.messenger.client.profile";

    // Keys: new list + legacy single image (kept for backward-compat)
    private static final String KEY_PHOTOS = "imagePaths";      // new: list (not wired yet)
    private static final String LEGACY_KEY_PHOTO = "imagePath"; // backward-compat
    ...
    private void buildUI() {
        ...
        avatar.setOnMouseClicked(e -> {
            List<String> uris = collectProfilePhotos();
            if (uris.isEmpty()) return;
            int start = 0;
            String saved = prefs.get("imagePath", null);
            if (saved != null && !saved.isBlank()) {
                String u = Path.of(saved).toUri().toString();
                int i = uris.indexOf(u);
                if (i >= 0) start = i;
            }
            PhotoViewer.show(uris, start);
        });
    ...
    private void loadFromSessionAndPrefs() {
        ...
        String saved = prefs.get(LEGACY_KEY_PHOTO, null); // keep legacy key working
    ...
    private void persistToPrefs() {
        ...
        if (pendingImagePath != null) {
            // keep using legacy single-image key until gallery is wired
            prefs.put(LEGACY_KEY_PHOTO, pendingImagePath);
        }
    }
    ...
    private List<String> collectProfilePhotos() {
        List<String> out = new ArrayList<>();
        try {
            Path dir = Path.of(System.getProperty("user.home"), ".yam", "profile");
            if (Files.isDirectory(dir)) {
                try (var s = Files.list(dir)) {
                    out = s.filter(p -> !Files.isDirectory(p))
                            .filter(p -> {
        ...
        // fallback to legacy single image if dir is empty
        if (out.isEmpty()) {
            String saved = prefs.get("imagePath", null);
            if (saved != null && !saved.isBlank()) out.add(Path.of(saved).toUri().toString());
        }
        return out;
    }
